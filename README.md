...
This project provides a Docker container for [Czkawka](https://github.com/qarmin/czkawka).
...
[Releases]: https://github.com/jlesage/docker-czkawka/releases
[Docker Hub]: https://hub.docker.com/r/jlesage/czkawka/tags
...
container, it would proxy all HTTP requests for `czkawka.domain.tld` to
...
server {
	...
	server_name czkawka.domain.tld;

	location / {
	        proxy_pass http://docker-czkawka;
	}
...
server {
	...
	location = /czkawka {return 301 $scheme://$http_host/czkawka/;}
	location /czkawka/ {
		proxy_pass http://docker-czkawka/;
		# Uncomment the following line if your Nginx server runs on a port that
		# differs from the one seen by external clients.
		#port_in_redirect off;
		location /czkawka/websockify {
			proxy_pass http://docker-czkawka/websockify;
			proxy_http_version 1.1;
			proxy_set_header Upgrade $http_upgrade;
			proxy_set_header Connection $connection_upgrade;
			proxy_read_timeout 86400;
		...
		# Needed when audio support is enabled.
		location /czkawka/websockify-audio {
			proxy_pass http://docker-czkawka/websockify-audio;
			proxy_http_version 1.1;
			proxy_set_header Upgrade $http_upgrade;
			proxy_set_header Connection $connection_upgrade;
			proxy_read_timeout 86400;
...
| **Application version** label | Displays the version of Czkawka integrated into Docker image. |
...
Any desktop notification generated by Czkawka is forwarded to the
